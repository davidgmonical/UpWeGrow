...
function scene:create(event)
  ...
  for row in db:nrows("SELECT * FROM timeslots") do
    ...
    slotData[tostring(d)] = {d, row.timestart, row.timeend}
  end
  ...
  for i = 1, 100 do
    ...
    local verticalLine = df:createRect(1, _H, 0, 0, i*100, 60, {0, 0.3, 0}, scrollView)
    
    local dayText = df:createDisplayText(days[date.wday], "paperdaisy-demo", 30, 0.5, 0, i*100 - 50, 0, scrollView, false, {0.4, 0.4, 0.4})
    local dateText = df:createDisplayText(date.day, "paperdaisy-demo", 24, 0.5, 0, i*100 - 50, 30, scrollView, false, {0.4, 0.4, 0.4})

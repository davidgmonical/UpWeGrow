...
function scene:create(event)
  ...
  local rect = df:createRect(_W, 120, 0, 0, 0, 61, {0.365, 0.69, 0.541}, sceneGroup)
    ...
    if (event.params.role == "g") then
      for row in db:nrows("SELECT * FROM gardeners WHERE id = 1") do
        answers = {row.name, row.location, row.years, row.bio}
      end
    else
      for row in db:nrows("SELECT * FROM landowners WHERE id = 1") do
